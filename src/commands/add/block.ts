...
// ── Inject into blocks.ts ─────────────────────────────────────────────────────

async function injectIntoBlocksTypes(blockName: string, pascalName: string) {
  ...
  const marker = '// (auto-generated by `cromatica add block` — do not edit manually)'

  if (content.includes(marker)) {
    content = content.replace(marker, `${marker}\n${blockTypeLine}\n${propsTypeLine}`)
  } else {
    // Fallback: append antes del bloque WithLocale
    content = content.replace(
...
export async function addBlock(blockName: string) {
  ...
  try {
    ...
    if (injected) {
      blocksTypesSpinner.succeed(`Injected ${chalk.green(`${pascalName}Props`)} into blocks.ts`)
    } else {
      blocksTypesSpinner.warn(
        `${chalk.yellow(`${pascalName}Props`)} was already in blocks.ts or file not found`,
      )
    }
  } catch (err) {
